...
I decided to represent it using a Sequence<Team>. This is because Sequence<Team> allows my PowerRanking component to not only allow users to switch teams' ranks manually, but also do it automatically for the user based on how each respective team's performance in the league compare to one another. Additionally, Sequence<Team> indexes variables within the Sequence, providing an automatic ordering system to base team ranks off of. This also allows for adding, removing, and retrieving items based on their rank. By convention, the sequence must be non-null, contain no duplicate teams, and remain in descending rank order — meaning the team at index i always
has a higher rank than the team at index j whenever i < j. The league name leagueName must also be a non-null string - empty strings are allowed. Finally, by correspondence, the team at sequence index i has rank i + 1 in the abstract PowerRanking. The first position at index 0 corresponds to the top-ranked team, and the league name leagueName corresponds directly to the stored String.
